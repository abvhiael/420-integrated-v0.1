...
test('AUT-12 requires live workers, registered jobs and scheduler/readiness evidence', () => {
  const report = buildAutomation12CloseoutReport420(environment(), evidence({
    liveWorkerCount: 0,
  ...
  assert.equal(report.decision, 'no-go');
  assert.ok(report.blockers.some((blocker) => blocker.includes('live Automation worker')));
  assert.ok(report.blockers.some((blocker) => blocker.includes('registered Automation job')));
