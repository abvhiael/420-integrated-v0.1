...
export class CoreProjectionConsumer420 implements BlockConsumer420 {
      ...
      await tx.query(
        `insert into idx_checkpoints(chain_id, block_number, block_hash, parent_hash)
         ...
         on conflict (chain_id) do update set
           ...
           block_hash=excluded.block_hash,
           parent_hash=excluded.parent_hash`,
        [batch.chainId.toString(), batch.block.number.toString(), batch.block.hash, batch.block.parentHash]
      );
    });
  }
  ...
  async rollbackTo(blockNumber: bigint | null): Promise<void> {
    await this.db.transaction(async (tx) => {
      ...
      await tx.query('delete from idx_checkpoints where block_number > $1', [n]);
    });
  }
}
