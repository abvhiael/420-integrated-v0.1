...
function block420(value: RpcBlock420 | null): IndexerBlock | null {
  ...
  return {
    number: quantity420(value.number),
    hash: value.hash,
    parentHash: value.parentHash,
    timestamp: quantity420(value.timestamp)
  };
}
...
function transaction420(value: RpcTransaction420 | null): IndexerTransaction | null {
  ...
  return {
    ...
    blockNumber: quantity420(value.blockNumber),
    transactionIndex: quantityNumber420(value.transactionIndex),
    from: value.from,
    to: value.to,
    input: value.input,
    value: quantity420(value.value)
  };
}
...
function log420(value: RpcLog420): IndexerLog {
  return {
    ...
    transactionIndex: quantityNumber420(value.transactionIndex),
    logIndex: quantityNumber420(value.logIndex),
    topics: value.topics,
    data: value.data,
...
export class EvmJsonRpcSource420 implements ChainSource420 {
  ...
  async chainId(): Promise<bigint> {
    return quantity420(await this.transport.request<Hex>('eth_chainId'));
  }

  async blockNumber(): Promise<bigint> {
    return quantity420(await this.transport.request<Hex>('eth_blockNumber'));
  }
  ...
  async getBlockByHash(blockHash: Hex): Promise<IndexerBlock | null> {
    return block420(await this.transport.request<RpcBlock420 | null>('eth_getBlockByHash', [blockHash, false]));
  }

  async getTransactionByHash(transactionHash: Hex): Promise<IndexerTransaction | null> {
    return transaction420(await this.transport.request<RpcTransaction420 | null>('eth_getTransactionByHash', [transactionHash]));
  }

  async getTransactionReceipt(transactionHash: Hex): Promise<IndexerReceipt | null> {
    return receipt420(await this.transport.request<RpcReceipt420 | null>('eth_getTransactionReceipt', [transactionHash]));
  }

  async getLogs(filter: LogFilter420): Promise<IndexerLog[]> {
    const rpcFilter: Record<string, unknown> = {
      fromBlock: quantityHex420(filter.fromBlock),
      toBlock: quantityHex420(filter.toBlock)
    };
    ...
    const logs = await this.transport.request<RpcLog420[]>('eth_getLogs', [rpcFilter]);
    return logs.map(log420);
  }

  call(request: { to: Hex; data: Hex }, blockNumber?: bigint): Promise<Hex> {
    return this.transport.request<Hex>('eth_call', [request, blockTag420(blockNumber)]);
  }

  getCode(address: Hex, blockNumber?: bigint): Promise<Hex> {
    return this.transport.request<Hex>('eth_getCode', [address, blockTag420(blockNumber)]);
  }
}
