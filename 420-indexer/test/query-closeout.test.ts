...
import { decodeAssetTransferCursor420, NATIVE_ASSET_TRANSFER_POSITION_420 } from '../src/query-layer.js';
...
test('asset transfer paging emits a stable composite cursor including native sentinel positions', async () => {
  const db = new FakeDb420();
  db.queue.push({ rows: [
    { block_number: '20', tx_hash: '0xbbb', log_index: 3 },
    { block_number: '20', tx_hash: '0xaaa', log_index: NATIVE_ASSET_TRANSFER_POSITION_420 }
  ] });
  ...
  assert.match(db.calls[0]!.sql, /order by block_number desc, lower\(tx_hash\) desc, log_index desc/);
});

test('asset transfer query accepts its own sentinel cursor for the next keyset page', async () => {
  const db = new FakeDb420();
  db.queue.push({ rows: [
    { block_number: '12', tx_hash: '0xbbb', log_index: NATIVE_ASSET_TRANSFER_POSITION_420 },
  ...
  assert.deepEqual(db.calls[1]!.params, ['420', '12', '0xbbb', NATIVE_ASSET_TRANSFER_POSITION_420, 2]);
});

test('public DTO mappers use JSON-safe strings and preserve the native sentinel', () => {
  ...
  assert.equal(assetTransferDto420({
    chain_id: '420', block_number: '9', tx_hash: '0xtx', log_index: NATIVE_ASSET_TRANSFER_POSITION_420, asset_key: 'native:420',
    asset_kind: 'native', contract_address: null, token_id: null, from_address: '0xfrom', to_address: '0xto', amount: '42'
  }).logIndex, NATIVE_ASSET_TRANSFER_POSITION_420);
});
