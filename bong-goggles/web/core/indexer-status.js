// Read-only diagnostic for the EXISTING 420-indexer /v1/status HTTP route.
// This response never grants app permissions or qualifies social projections.
...
export function validateIndexerStatusEnvelope(envelope,chainId){
  ...
  if(!s.finality||!['head','confirmations','finalized'].includes(s.finality.mode)||!unsignedOrNull(s.finality.confirmations)||!unsignedOrNull(s.finality.safeHead))return null;
