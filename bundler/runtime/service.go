...
func (s *Service) Qualify(ctx context.Context, now time.Time) error {
	...
	if code == "" || code == "0x" || code == "0x0" {
		s.ready.Store(false)
		return errors.New("entry point has no deployed code")
	}

	headTime, err := s.probe.LatestBlockTime(ctx)
	if err != nil { s.ready.Store(false); return fmt.Errorf("latest block unavailable: %w", err) }
	if headTime.IsZero() || headTime.After(now.Add(time.Second)) || now.Sub(headTime) > s.cfg.MaxHeadAge {
		s.ready.Store(false)
		return fmt.Errorf("execution head is stale or invalid: head_time=%s", headTime.UTC().Format(time.RFC3339))
	}
	s.headUnix.Store(headTime.Unix())
	s.ready.Store(true)
	return nil
}
...
func (s *Service) Handler() http.Handler {
	...
	mux.HandleFunc("GET /healthz", func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(w, http.StatusOK, map[string]any{
			"service":"420Bundler","status":"ok","canonical":false,
			"custodial":false,"authorization_authority":false,
		})
	})
	mux.HandleFunc("GET /readyz", func(w http.ResponseWriter, _ *http.Request) {
		if !s.ready.Load() {
			writeJSON(w, http.StatusServiceUnavailable, map[string]any{"service":"420Bundler","ready":false,"canonical":false})
			return
		}
		writeJSON(w, http.StatusOK, map[string]any{
			"service":"420Bundler","ready":true,"canonical":false,
			"chain_id":s.cfg.ChainID,"entry_point":s.cfg.EntryPoint,
			"head_observed_at":time.Unix(s.headUnix.Load(),0).UTC().Format(time.RFC3339),
		})
	})
	return mux
}
