...
/// @notice First-party Crash V1 wager-scoped session and deterministic crash engine.
/// @dev E2.6 hardening makes V1 auto-cashout-only because canonical randomness is public before
///      session activation. A precommitted auto target cannot be adapted after observing the root.
contract CrashV1420 is ICasinoGame420 {
    ...
    uint64 public constant GROWTH_BPS_PER_SECOND = 1_000;
    uint64 public constant MAX_CRASH_BPS = 1_000_000_000;
    ...
    struct Params { uint64 autoCashoutBps; }
    ...
    function startSession(bytes32 wagerId, Params calldata params) external {
        ...
        if (params.autoCashoutBps == 0) revert ManualCashoutDisabled();
        if (wager.stake == 0 || uint256(params.autoCashoutBps) > type(uint256).max / wager.stake) revert InvalidLiability();
        uint256 committedGross = (wager.stake * uint256(params.autoCashoutBps)) / uint256(BPS);
        if (committedGross > wager.maxGrossPayout) revert InvalidLiability();
    ...
    function advance(bytes32 wagerId) public returns (TerminalReason reason) {
        ...
        if (session.autoCashoutBps < session.crashPointBps && live >= session.autoCashoutBps) {
    ...
    /// @dev Manual cash-out is deliberately disabled for V1 while randomness roots are public pre-session.
    function cashOut(bytes32 wagerId) external returns (uint64) {
        SessionState storage session = _active(wagerId);
        if (msg.sender != session.player) revert NotPlayer();
        revert ManualCashoutDisabled();
    }
