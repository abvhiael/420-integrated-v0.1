...
contract RandomnessRouter420 is I420System {
    ...
    event RandomnessProfileConfigured(
        bytes32 indexed profileId,
        Method method,
        address indexed primaryProvider,
    ...
    constructor(address authorization_, address profileRegistry_, address wagerRegistry_) {
        if (authorization_ == address(0) || profileRegistry_ == address(0) || wagerRegistry_ == address(0)) {
            revert ZeroAddress();
        }
    ...
    function configureProfile(RandomnessProfile calldata input) external {
        ...
        if (
            input.method == Method.NONE || input.primaryProvider == address(0) || input.fallbackDelay == 0
                || input.securityLevelHash == bytes32(0) || input.domainSeparator == bytes32(0)
                || input.manifestHash == bytes32(0)
        ) revert InvalidConfiguration();
        if (input.fallbackProvider == input.primaryProvider && input.fallbackProvider != address(0)) {
            revert InvalidConfiguration();
        }
        if (
            !authorization.isAuthorized(
                ...
                authorization.scopeForProfile(input.profileId),
                0
            )
        ) revert Unauthorized();

        RandomnessProfile memory copy = input;
        copy.exists = true;
        _profiles[input.profileId] = copy;
        emit RandomnessProfileConfigured(
            copy.profileId,
            copy.method,
            copy.primaryProvider,
            copy.fallbackProvider,
            copy.fallbackDelay,
            copy.securityLevelHash,
            copy.domainSeparator,
            copy.manifestHash
        );
    }
    ...
    function requestRandomness(bytes32 wagerId, bytes32 contextHash) external returns (uint64 fallbackAt) {
        ...
        fallbackAt = requestedAt + profile.fallbackDelay;
        _requests[wagerId] = RandomnessRequest({
            wagerId: wagerId,
    ...
    function fulfillPrimary(bytes32 wagerId, bytes32 entropy, bytes32 proofHash) external returns (bytes32 root) {
        root = _fulfill(wagerId, entropy, proofHash, Source.PRIMARY);
    }

    function fulfillFallback(bytes32 wagerId, bytes32 entropy, bytes32 proofHash) external returns (bytes32 root) {
        root = _fulfill(wagerId, entropy, proofHash, Source.FALLBACK);
    }

    function getProfile(bytes32 profileId) external view returns (RandomnessProfile memory) {
        return _getProfile(profileId);
    }

    function getRequest(bytes32 wagerId) external view returns (RandomnessRequest memory request) {
    ...
    function rootOf(bytes32 wagerId) external view returns (bytes32 root) {
        ...
        if (!request.fulfilled) revert NotFound();
        return request.root;
    }

    function _fulfill(bytes32 wagerId, bytes32 entropy, bytes32 proofHash, Source source)
        private
        returns (bytes32 root)
    {
        ...
        if (request.fulfilled) {
            ...
            if (request.source == source && request.proofHash == proofHash && request.entropyHash == entropyHash) {
                return request.root;
            }
        ...
        bytes32 entropyHash_ = keccak256(abi.encode(entropy));
        root = keccak256(
            abi.encode(
                ROOT_DOMAIN,
                block.chainid,
        ...
        request.entropyHash = entropyHash_;
        request.source = source;
        request.fulfilled = true;
        emit RandomnessFulfilled(wagerId, request.profileId, source, root, proofHash, entropyHash_);
    }

    function _getProfile(bytes32 profileId) private view returns (RandomnessProfile storage profile) {
        profile = _profiles[profileId];
        if (!profile.exists) revert NotFound();
    }
}
