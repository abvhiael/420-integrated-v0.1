...
import "./RouletteV1420.sol";

/// @notice Read-only reconstruction surface for the first-party RouletteV1 game.
/// @dev This contract has no custody, randomness, settlement, or admin authority.
contract RouletteV1View420 is I420System {
    struct Snapshot {
        ...
        RouletteV1420.Result result;
    }

    BetRegistry420 public immutable wagerRegistry;
    RandomnessRouter420 public immutable randomnessRouter;
    RouletteV1420 public immutable roulette;

    error ZeroAddress();
    error DependencyMismatch();

    constructor(address wagerRegistry_, address randomnessRouter_, address roulette_) {
        if (wagerRegistry_ == address(0) || randomnessRouter_ == address(0) || roulette_ == address(0)) {
            revert ZeroAddress();
        }

        wagerRegistry = BetRegistry420(wagerRegistry_);
        randomnessRouter = RandomnessRouter420(randomnessRouter_);
        roulette = RouletteV1420(roulette_);

        if (
            address(roulette.wagerRegistry()) != wagerRegistry_
                || address(roulette.randomnessRouter()) != randomnessRouter_
        ) revert DependencyMismatch();
    }

    function systemName() external pure returns (string memory) { return "RouletteV1View420"; }
    function protocolVersion() external pure returns (uint32) { return 1; }

    function snapshot(bytes32 wagerId, RouletteV1420.Params calldata params)
        external
        view
        returns (Snapshot memory s)
    {
        s.wager = wagerRegistry.getWager(wagerId);
        s.paramsMatch = roulette.hashParams(params) == s.wager.paramsHash;
        ...
        if (s.paramsMatch && s.randomness.fulfilled) {
            try roulette.resolve(wagerId, params) returns (RouletteV1420.Result memory result) {
