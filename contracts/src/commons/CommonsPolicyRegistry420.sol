...
contract CommonsPolicyRegistry420 is SystemAccess, I420System {
    struct Policy {
        bytes32 policyType;
    ...
    function systemName() external pure returns (string memory) { return "CommonsPolicyRegistry420"; }
    ...
    function setPolicy(
        ...
        if (policyType == bytes32(0)) revert InvalidPolicyType();
        if (semanticsHash == bytes32(0)) revert InvalidSemanticsHash();

        Policy storage policy = _policies[policyId];
        if (policy.exists && (policy.policyType != policyType || policy.semanticsHash != semanticsHash)) {
            revert PolicySemanticChange();
        }
        uint32 nextRevision = policy.exists ? policy.revision + 1 : 1;
        ...
        policy.exists = true;
        emit PolicyConfigured(policyId, policyType, semanticsHash, metadataHash, nextRevision, active);
    }
