...
/// @notice The production verifier-evidence implementation: an attested independent appointment
/// is mandatory before any positive OR negative signed decision is recorded.
/// @dev Deploy and configure THIS adapter rather than the signature-only test primitive.
contract ComputeJobPolicyEnforcedVerification420 is ComputeJobIndependentVerification420 {
    ...
    function submitVerdict(Verdict calldata v, bytes calldata signature)
        public override returns (bytes32 decisionRef)
    {
        ...
        // Fetch payer from canonical signed request rather than trusting an arbitrary caller-provided address.
