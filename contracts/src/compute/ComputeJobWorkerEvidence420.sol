...
/// @notice On-chain operator-accepted assignment and worker-authenticated receipt
/// commitments. Not hardware attestation, reserved capacity, metering or correctness.
contract ComputeJobWorkerEvidence420 is IComputeJobWorkerEvidence420 {
    ...
    constructor(address resources_, address authorization_) {
        if (resources_.code.length == 0 || authorization_.code.length == 0) revert InvalidEvidence();
    ...
    /// @dev Bound once by the deployment principal only; the registry must
    /// have been constructed with this exact evidence gateway as its worker authority.
    function bindJobs(address jobs_) external {
        if (msg.sender != bindingAdmin || address(jobs) != address(0) || jobs_.code.length == 0)
            revert Unauthorized();
        if (address(ComputeJobRegistry420(jobs_).workerEvidence()) != address(this)) revert InvalidEvidence();
    ...
    /// @notice Worker authenticates the receipt using its direct chain transaction;
    /// worker then directly calls jobs.recordResult to preserve msg.sender identity.
