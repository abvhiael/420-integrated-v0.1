...
contract ExchangeAuthorization420 {
    ...
    function canSwap(address principal, bytes32 marketId, uint256 amount) external view returns (bool) {
        return capabilityRegistry.isAuthorized(principal, ExchangeIds420.EXCHANGE_ROUTER, ExchangeIds420.ACTION_SWAP, scopeMarket(marketId), amount);
    }

    function canPlaceLimitOrder(address principal, bytes32 marketId, uint256 amount) external view returns (bool) {
        return capabilityRegistry.isAuthorized(principal, ExchangeIds420.MARKET_REGISTRY, ExchangeIds420.ACTION_LIMIT_ORDER, scopeMarket(marketId), amount);
    }

    function canBridgeDeposit(address principal, bytes32 assetId, uint256 amount) external view returns (bool) {
        return capabilityRegistry.isAuthorized(principal, ExchangeIds420.ASSET_REGISTRY, ExchangeIds420.ACTION_BRIDGE_DEPOSIT, scopeAsset(assetId), amount);
    }

    function canBridgeWithdraw(address principal, bytes32 assetId, uint256 amount) external view returns (bool) {
        return capabilityRegistry.isAuthorized(principal, ExchangeIds420.ASSET_REGISTRY, ExchangeIds420.ACTION_BRIDGE_WITHDRAW, scopeAsset(assetId), amount);
    }
}
