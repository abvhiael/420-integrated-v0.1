...
library ServiceIds420 {
    ...
    function isGenesisCanonical(bytes32 serviceId) internal pure returns (bool) {
        return serviceId == PROTOCOL_REGISTRY || serviceId == WALLET || serviceId == SMART_ACCOUNTS || serviceId == INTEROP || serviceId == RANDOMNESS || serviceId == EXPLORER || serviceId == SEARCH || serviceId == ANALYTICS || serviceId == APPSTORE || serviceId == VERIFY || serviceId == NOTIFICATIONS || serviceId == NAMES || serviceId == IDENTITY || serviceId == ARBITRATION || serviceId == TRUST || serviceId == COMMONS || serviceId == PULSE || serviceId == MESSENGER || serviceId == VAULT || serviceId == TREASURY || serviceId == DEVELOPMENT_COMPENSATION || serviceId == GRANTS || serviceId == LAUNCHPAD || serviceId == TOKEN || serviceId == RESOURCE_PROTOCOL || serviceId == RELAY || serviceId == STORE || serviceId == CACHE || serviceId == GATEWAY || serviceId == MARKET || serviceId == RIGHTS || serviceId == SWAP || serviceId == PAY || serviceId == ORACLE || serviceId == BRIDGE || serviceId == STAKE || serviceId == CIVIC || serviceId == GOVERNANCE || serviceId == AI || serviceId == COMPUTE_MARKET || serviceId == ATTENTION || serviceId == CANNASEUR || serviceId == STATUS;
    }
}
