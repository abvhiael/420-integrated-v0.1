...
contract ResourceSessionRegistry420 is I420System {
    ...
    ResourceOfferRegistry420 public immutable offers; mapping(bytes32=>Session) private _sessions;
    error InvalidSession(); error SessionExists(); error Unauthorized(); error InvalidState();
    constructor(address offers_){require(offers_!=address(0),"dependency");offers=ResourceOfferRegistry420(offers_);}
    ...
    function openSession(bytes32 sessionId,bytes32 offerId,uint128 maxUnits,uint64 expiresAt) external { if(sessionId==0||offerId==0||maxUnits==0||expiresAt<=block.timestamp||!offers.isEffective(offerId)) revert InvalidSession(); if(_sessions[sessionId].exists) revert SessionExists(); ResourceOfferRegistry420.Offer memory o=offers.getOffer(offerId); if(maxUnits>o.maxUnits) revert InvalidSession(); _sessions[sessionId]=Session(offerId,msg.sender,maxUnits,uint256(maxUnits)*uint256(o.unitPrice420),uint64(block.timestamp),expiresAt,State.OPEN,true); }
    ...
    function markSettled(bytes32 sessionId) external { Session storage s=_get(sessionId); require(msg.sender!=address(0)); if(s.state!=State.CLOSED) revert InvalidState(); s.state=State.SETTLED; }
