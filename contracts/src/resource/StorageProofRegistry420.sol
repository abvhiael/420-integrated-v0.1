...
contract StorageProofRegistry420 is I420System {
    ...
    struct ProofReceipt { bytes32 commitmentId; bytes32 challengeId; bytes32 proofDigest; uint64 challengeEpoch; uint64 submittedAt; bool exists; }
    ...
    error ZeroAddress(); error InvalidProof(); error ProofExists(); error ChallengeReplayed(); error ChallengeOutsideCommitment(); error ProofTooEarly(); error ProofTooLate(); error InactiveStoreNode(); error InactiveProofScheme(); error VerificationFailed(); error Reentrancy();
    ...
    modifier nonReentrant() { if (_entered != 0) revert Reentrancy(); _entered = 1; _; _entered = 0; }
    constructor(address commitments_, address schemes_, address nodes_) {
        ...
        commitments = StorageCommitmentRegistry420(commitments_); schemes = StorageProofSchemeRegistry420(schemes_); nodes = ResourceNodeRegistry420(nodes_);
    }
    function systemName() external pure returns (string memory) { return "StorageProofRegistry420"; }
    function protocolVersion() external pure returns (uint32) { return 1; }
    function canonicalProofId(bytes32 commitmentId, bytes32 challengeId, bytes32 proofDigest) public pure returns (bytes32) { return keccak256(abi.encode("420/STORAGE/PROOF_RECEIPT/V1", commitmentId, challengeId, proofDigest)); }
    function submitProof(bytes32 commitmentId, bytes32 challengeId, uint64 challengeEpoch, bytes calldata proof) external nonReentrant returns (bytes32 proofId) {
        ...
        bool verified;
        try IStorageProofVerifier420(scheme.verifier).verifyStorageProof(commitment.proofSchemeId, commitmentId, commitment.providerId, commitment.nodeId, commitment.contentRoot, commitment.replicaRoot, challengeId, challengeEpoch, proof) returns (bool ok) { verified = ok; } catch { revert VerificationFailed(); }
    ...
    function getProof(bytes32 proofId) external view returns (ProofReceipt memory receipt) { receipt = _proofs[proofId]; if (!receipt.exists) revert InvalidProof(); }
    function hasVerifiedChallenge(bytes32 commitmentId, bytes32 challengeId) external view returns (bool) { return challengeConsumed[commitmentId][challengeId]; }
}
