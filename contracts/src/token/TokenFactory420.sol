...
contract TokenFactory420 is I420System {
    ...
    error ZeroAddress(); error IncorrectFee(); error TemplateDisabled(); error InvalidTemplate(); error FeeTransferFailed(); error Reentrancy(); error InvalidConfig();
    ...
    constructor(address templateRegistry_,address payable feeReceiver_){if(templateRegistry_==address(0)||feeReceiver_==address(0)) revert ZeroAddress();templates=TokenTemplateRegistry420(templateRegistry_);feeReceiver=feeReceiver_;}
    ...
    function _finish(address token,bytes32 id,bytes32 configHash) internal {TokenTemplateRegistry420.Template memory meta=templates.template(id);isFactoryDeployment[token]=true;_deployments.push(Deployment(token,msg.sender,id,meta.version,configHash,uint64(block.timestamp)));emit TokenDeployed(token,msg.sender,id,meta.version,configHash,CREATION_FEE);(bool ok,)=feeReceiver.call{value:CREATION_FEE}("");if(!ok) revert FeeTransferFailed();}
