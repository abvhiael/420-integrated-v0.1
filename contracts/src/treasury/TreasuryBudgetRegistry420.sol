...
contract TreasuryBudgetRegistry420 is I420System, SystemAccess {
    ...
    error UnauthorizedCaller(); error InvalidBudget(); error BudgetExists(); error BudgetNotFound(); error BudgetExceeded();
    ...
    function reserve(bytes32 budgetId, uint128 amount) external { Budget storage b = _get(budgetId); if (msg.sender != governanceTimelock) revert UnauthorizedCaller(); if (!_effective(b) || uint256(b.committed) + amount > b.ceiling) revert BudgetExceeded(); b.committed += amount; emit BudgetCommitmentChanged(budgetId,b.committed,b.executed); }
    function settle(bytes32 budgetId, uint128 amount) external { Budget storage b = _get(budgetId); if (msg.sender != governanceTimelock) revert UnauthorizedCaller(); if (amount > b.committed - b.executed) revert BudgetExceeded(); b.executed += amount; emit BudgetCommitmentChanged(budgetId,b.committed,b.executed); }
    function release(bytes32 budgetId, uint128 amount) external { Budget storage b = _get(budgetId); if (msg.sender != governanceTimelock) revert UnauthorizedCaller(); if (amount > b.committed - b.executed) revert BudgetExceeded(); b.committed -= amount; emit BudgetCommitmentChanged(budgetId,b.committed,b.executed); }
