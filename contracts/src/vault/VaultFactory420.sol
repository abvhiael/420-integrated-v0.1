...
contract VaultFactory420 is I420System {
    ...
    constructor(address registry_, address authorization_, address accounting_) {
        if (registry_ == address(0) || authorization_ == address(0) || accounting_ == address(0)) revert ZeroAddress();
        registry = registry_;
        authorization = authorization_;
        accounting = accounting_;
    }

    function systemName() external pure returns (string memory) { return "VaultFactory420"; }
    function protocolVersion() external pure returns (uint32) { return 1; }

    function deployVault(bytes32 vaultId, bytes32 salt) external returns (address vaultAddress) {
        AssetVault420 vault = new AssetVault420{salt: keccak256(abi.encode(msg.sender, vaultId, salt))}(
            vaultId,
            registry,
            authorization,
            accounting,
