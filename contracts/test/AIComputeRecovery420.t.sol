...
contract MockAIComputeCapabilities420 is ICapabilityRegistry420 {
    ...
    function grant(bytes32) external pure returns (CapabilityGrant memory capabilityGrant) {
...
contract AIComputeRecovery420Test is Test {
    ...
    function setUp() public {
        ...
        offers = new ComputeOfferRegistry420(
            address(auth), address(providers), address(nodes), address(resources), address(policy)
        );
        ...
        resources.registerResource(
            RESOURCE_ID,
            NODE_ID,
            ComputeIds420.CLASS_GPU_INFERENCE,
            keccak256("gpu-profile"),
            keccak256("runtime-profile"),
        ...
        requests.createRequest(
            REQUEST_ID,
            ComputeIds420.WORKLOAD_AI_INFERENCE,
            keccak256("resource-requirement"),
            REQUEST_HASH,
            100,
            uint64(block.timestamp + 1 hours),
            PRIVACY_POLICY,
            VERIFY_PROFILE,
            keccak256("provider-constraints")
        );
        vm.prank(USER);
        ...
        caps.set(
            ...
            ComputeIds420.ACTION_ACCEPT_MATCH,
            auth.scopeRequest(REQUEST_ID),
            100,
            true
        );
        ...
        computeJobs = new ComputeJobRegistry420(
            address(auth), address(matches), address(providers), address(requests)
        );
        ...
        vm.startPrank(PROVIDER);
        aiProviders.registerProvider(
            AI_PROVIDER_ID,
            PROVIDER,
            PROVIDER,
            keccak256("ai-provider-manifest"),
            keccak256("ai-stake"),
            COMPUTE_PROVIDER_ID
        );
        ...
        adapter = new AIComputeAdapter420(
            address(aiJobs),
            address(aiProviders),
            address(requests),
            address(matches),
    ...
    function testEndToEndAIJobBindsAndSynchronizesComputeLifecycle() public {
        ...
        caps.set(
            USER,
            ComputeIds420.COMPONENT_COMPUTE,
            ComputeIds420.ACTION_VERIFY,
            auth.scopeJob(computeJobId),
            0,
            true
        );
        vm.prank(USER);
