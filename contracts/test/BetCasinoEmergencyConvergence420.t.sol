...
contract MockCasinoEmergencyRisk420 {
    ...
    function releaseExposure(bytes32) external returns (uint256) { require(!released, "released"); released = true; return 400 ether; }
}
contract MockCasinoEmergencyVault420 {
    ...
    // Mirror BankrollVault420.resolveWager return ABI so the engine can decode the call.
    function resolveWager(bytes32, uint256 grossPayout) external returns (uint256 stake, uint256 stakeAbsorbed, uint256 bankrollOutflow) {
        require(!resolved, "resolved"); resolved = true; payout = grossPayout;
        stake = 100 ether;
...
contract MockCasinoEmergencyEconomics420 {
    ...
    function finalizeWagerFees(bytes32, BetTypes420.TerminalOutcome outcome_) external {
        require(!finalized, "finalized"); finalized = true; outcome = outcome_;
    }
}
...
contract BetCasinoEmergencyConvergence420Test {
    ...
    function _versions() private pure returns (bytes32[8] memory versions) {
        versions = [DICE_V1, KENO_V1, PLINKO_V1, SLOT_V1, ROULETTE_V1, BLACKJACK_V1, MINES_V1, CRASH_V1];
    }
    ...
    function _deploy(bytes32 gameVersionId, uint256 nonce) private returns (Suite memory s) {
        ...
        s.deadline = nowTs + 1 hours;
        s.registry.setWager(BetTypes420.Wager({
            wagerId:s.wagerId, player:PLAYER, operatorId:keccak256("operator"),
            gameId:keccak256(abi.encode("game", gameVersionId)), gameVersionId:gameVersionId,
            asset:ASSET, stake:100 ether, maxGrossPayout:500 ether,
            paramsHash:keccak256(abi.encode("params", gameVersionId)), vaultId:VAULT,
            randomnessProfileId:keccak256(abi.encode("randomness", gameVersionId)),
            riskProfileId:keccak256(abi.encode("risk", gameVersionId)), settlementProfileId:s.settlementProfile,
            accessPolicyId:keccak256(abi.encode("access", gameVersionId)),
            rulesetId:keccak256(abi.encode("ruleset", gameVersionId)),
            acceptedAt:nowTs, deadline:s.deadline, status:BetTypes420.WagerStatus.ACCEPTED
        }));
        ...
        _allow(s, ADMIN, BetIds420.ACTION_EMERGENCY_SET,
    ...
    function testSettlementHoldBlocksEconomicOutcomeAcrossCasinoGames() public {
        ...
        for (uint256 i = 0; i < versions.length; ++i) {
            ...
            _haltSettlement(s);
            vm.expectRevert(abi.encodeWithSelector(SettlementEngine420.EmergencyHalted.selector,
                BetTypes420.EmergencyDomain.SETTLEMENT_HOLD, s.settlementProfile));
            vm.prank(SETTLER);
            s.engine.settle(s.wagerId, BetTypes420.TerminalOutcome.WIN, 500 ether);
            require(!s.registry.settlementExists(s.wagerId) && !s.risk.released()
                && !s.vault.resolved() && !s.economics.finalized(), "halt leak");
        }
    }
    function testSettlementHoldNeverStrandsExpiredVoidAcrossCasinoGames() public {
        ...
        for (uint256 i = 0; i < versions.length; ++i) {
            ...
            require(settlement.outcome == BetTypes420.TerminalOutcome.VOID && settlement.grossPayout == 100 ether, "void");
            require(s.risk.released() && s.vault.resolved() && s.vault.payout() == 100 ether
                && s.economics.finalized() && s.economics.outcome() == BetTypes420.TerminalOutcome.VOID, "stranded");
        }
    }
    function testEmergencyKeysAreDomainSeparatedForCrashGameAndVersion() public {
        Suite memory s = _deploy(CRASH_V1, 999);
        bytes32 gameId = keccak256(abi.encode("game", CRASH_V1));
        bytes32 gameKey = s.emergency.emergencyKey(BetTypes420.EmergencyDomain.GAME, gameId);
        bytes32 versionKey = s.emergency.emergencyKey(BetTypes420.EmergencyDomain.GAME_VERSION, CRASH_V1);
        bytes32 settlementKey = s.emergency.emergencyKey(BetTypes420.EmergencyDomain.SETTLEMENT_HOLD, s.settlementProfile);
        require(gameKey != versionKey && gameKey != settlementKey && versionKey != settlementKey, "emergency collision");
    }
}
