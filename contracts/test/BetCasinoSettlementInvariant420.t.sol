...
interface VmBetCasinoSettlement420 {
...
contract MockCasinoSettlementRisk420 {
...
contract MockCasinoSettlementEconomics420 {
    bool public finalized;
    function finalizeWagerFees(bytes32, BetTypes420.TerminalOutcome) external {
...
contract BetCasinoSettlementInvariant420Test {
    ...
    bytes32 constant VAULT = keccak256("casino/convergence/vault");
    bytes32 constant SETTLEMENT_PROFILE = keccak256("casino/convergence/settlement");
    ...
    struct Suite {
        MockCapabilityRegistryCasinoSettlement420 caps;
        BetAuthorization420 auth;
        MockCasinoSettlementRegistry420 registry;
        MockCasinoSettlementRisk420 risk;
        MockCasinoSettlementVault420 vault;
        MockCasinoSettlementEconomics420 economics;
        SettlementEngine420 engine;
        bytes32 wagerId;
    }

    function _deploy(bytes32 gameVersionId, uint256 nonce) private returns (Suite memory s) {
        s.caps = new MockCapabilityRegistryCasinoSettlement420();
        s.auth = new BetAuthorization420(address(s.caps));
        s.registry = new MockCasinoSettlementRegistry420();
        s.risk = new MockCasinoSettlementRisk420();
        s.vault = new MockCasinoSettlementVault420(VAULT, ASSET);
        s.economics = new MockCasinoSettlementEconomics420();
    ...
    function testMaxGrossPayoutBoundaryIsIdenticalAcrossCasinoGames() public {
        ...
        for (uint256 i = 0; i < versions.length; ++i) {
            ...
            s.engine.settle(s.wagerId, BetTypes420.TerminalOutcome.WIN, 501 ether);
    ...
    function testAuthorizedTerminalSettlementAndRetryAreIdenticalAcrossCasinoGames() public {
        ...
        for (uint256 i = 0; i < versions.length; ++i) {
            ...
            vm.prank(SETTLER);
            BetTypes420.Settlement memory first = s.engine.settle(s.wagerId, BetTypes420.TerminalOutcome.WIN, 500 ether);

            require(first.wagerId == s.wagerId, "wrong wager");
            require(first.outcome == BetTypes420.TerminalOutcome.WIN, "wrong outcome");
            require(first.grossPayout == 500 ether, "wrong payout");
            require(s.risk.released(), "risk not released");
            require(s.vault.resolved(), "vault not resolved");
            require(s.vault.payout() == 500 ether, "vault payout");
            require(s.economics.finalized(), "economics not finalized");

            vm.prank(SETTLER);
            BetTypes420.Settlement memory retry = s.engine.settle(s.wagerId, BetTypes420.TerminalOutcome.WIN, 500 ether);
    ...
    function testConflictingSecondTerminalOutcomeFailsAcrossCasinoGames() public {
        bytes32[6] memory versions = _versions();
        for (uint256 i = 0; i < versions.length; ++i) {
            Suite memory s = _deploy(versions[i], i + 200);
            vm.prank(SETTLER);
            ...
            s.engine.settle(s.wagerId, BetTypes420.TerminalOutcome.LOSS, 0);

            BetTypes420.Settlement memory settlement = s.registry.getSettlement(s.wagerId);
            require(settlement.outcome == BetTypes420.TerminalOutcome.WIN, "terminal outcome changed");
            require(settlement.grossPayout == 500 ether, "terminal payout changed");
        }
    }
}
