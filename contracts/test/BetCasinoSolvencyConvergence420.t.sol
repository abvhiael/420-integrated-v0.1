...
contract BetCasinoSolvencyConvergence420Test {
    ...
    function testEightCasinoGamesShareOneSolventLiabilityPool() public {
        Suite memory s = _deploy(); bytes32[8] memory versions = _versions();
        ...
        require(state.totalAssets==1_000 ether,"assets"); require(state.activeReservedLiability==800 ether,"reserved"); require(state.safetyReserve==100 ether,"safety"); require(s.risk.reservedByVault(VAULT)==800 ether,"risk vault"); require(s.accounting.availableForNewRisk(VAULT)==100 ether,"available");
    ...
    function testLargeWinAccountingCannotConsumeOtherGamesProtectedLiability() public {
        Suite memory s=_deploy(); bytes32[8] memory versions=_versions(); bytes32[8] memory wagers; for(uint256 i=0;i<versions.length;++i) wagers[i]=_reserve(s,versions[i],i+300,100 ether); s.risk.releaseExposure(wagers[7]); vm.expectRevert(VaultAccounting420.Insolvent.selector); s.accounting.recordWagerSettlement(VAULT,0,301 ether); VaultAccounting420.VaultState memory state=s.accounting.getVault(VAULT); require(state.totalAssets==1_000 ether,"assets changed"); require(state.activeReservedLiability==700 ether,"liabilities changed"); require(state.safetyReserve==100 ether,"safety changed"); require(state.realizedPnl==0,"pnl changed");
    }
}
