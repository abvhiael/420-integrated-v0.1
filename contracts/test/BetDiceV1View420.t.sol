...
import "../src/bet/DiceV1View420.sol";
import "../src/bet/RandomnessRouter420.sol";

contract MockDiceV1ViewRegistry420 {
...
contract MockDiceV1ViewRandomness420 {
...
contract BetDiceV1View420Test {
    bytes32 constant WAGER = keccak256("wager/dice/client-view");
    ...
    struct Suite {
        MockDiceV1ViewRegistry420 registry;
        MockDiceV1ViewRandomness420 randomness;
        DiceV1420 dice;
        DiceV1View420 view420;
        DiceV1420.Params params;
    }

    function _deploy() private returns (Suite memory s) {
        s.registry = new MockDiceV1ViewRegistry420();
        s.randomness = new MockDiceV1ViewRandomness420();
        s.dice = new DiceV1420(address(s.registry), address(s.randomness), GAME, GAME_V1, RULESET);
        s.view420 = new DiceV1View420(address(s.registry), address(s.randomness), address(s.dice));
        s.params = DiceV1420.Params({rollUnder: true, threshold: 5000, winGrossPayout: 190 ether});
        ...
        s.registry.setWager(BetTypes420.Wager({
            ...
            paramsHash: s.dice.hashParams(s.params),
    ...
    function _fulfill(Suite memory s) private returns (bytes32 root) {
        root = keccak256("canonical-root/client-view");
        s.randomness.setRequest(RandomnessRouter420.RandomnessRequest({
            wagerId: WAGER,
            profileId: RANDOMNESS,
            gameVersionId: GAME_V1,
            paramsHash: s.dice.hashParams(s.params),
            contextHash: keccak256("draw-context/dice/v1"),
            requestedAt: 110,
            fallbackAt: 210,
            root: root,
    ...
    function testSnapshotShowsAcceptedWagerBeforeRandomness() public {
        ...
        DiceV1View420.Snapshot memory snap = s.view420.snapshot(WAGER, s.params);
    ...
    function testSnapshotShowsFulfilledRandomnessAndReproducibleSettlement() public {
        ...
        DiceV1420.Result memory result = s.dice.resolve(WAGER, s.params);
        ...
        require(snap.result.roll == result.roll, "roll changed");
    ...
    function testSnapshotFlagsTamperedParamsWithoutInventingResult() public {
        Suite memory s = _deploy();
        _fulfill(s);
        DiceV1420.Params memory altered = s.params;
        altered.threshold = 4999;

        DiceV1View420.Snapshot memory snap = s.view420.snapshot(WAGER, altered);
