...
import "../src/bet/PlinkoV1View420.sol";

contract MockPlinkoV1ViewRegistry420 {
...
contract MockPlinkoV1ViewRandomness420 {
...
contract BetPlinkoV1View420Test {
    bytes32 constant WAGER = keccak256("wager/plinko/client-view");
    ...
    struct Suite {
        MockPlinkoV1ViewRegistry420 registry;
        MockPlinkoV1ViewRandomness420 randomness;
        PlinkoV1420 plinko;
        PlinkoV1View420 view420;
    ...
    function _deploy() private returns (Suite memory s) {
        s.registry = new MockPlinkoV1ViewRegistry420();
        s.randomness = new MockPlinkoV1ViewRandomness420();
        s.plinko = new PlinkoV1420(address(s.registry), address(s.randomness), GAME, GAME_V1, RULESET);
        s.view420 = new PlinkoV1View420(address(s.registry), address(s.randomness), address(s.plinko));
        s.params = _params();
    ...
    function _fulfill(Suite memory s) private returns (bytes32 root) {
        root = keccak256("canonical-root/plinko-view");
        s.randomness.setRequest(RandomnessRouter420.RandomnessRequest({
            wagerId: WAGER,
            profileId: RANDOMNESS,
            gameVersionId: GAME_V1,
            paramsHash: s.plinko.hashParams(s.params),
            contextHash: keccak256("path-context/plinko/v1"),
            requestedAt: 110,
            fallbackAt: 210,
            root: root,
    ...
    function testSnapshotShowsAcceptedWagerBeforeRandomness() public {
        ...
        PlinkoV1View420.Snapshot memory snap = s.view420.snapshot(WAGER, s.params);
    ...
    function testSnapshotShowsFulfilledRandomnessAndReproducibleSettlement() public {
        ...
        PlinkoV1420.Result memory result = s.plinko.resolve(WAGER, s.params);
        ...
        require(snap.result.pathBits == result.pathBits, "path changed");
        require(snap.result.rightMoves == result.rightMoves, "right count changed");
        require(snap.result.bucket == result.bucket, "bucket changed");
    ...
    function testSnapshotFlagsTamperedParamsWithoutInventingResult() public {
        ...
        PlinkoV1View420.Snapshot memory snap = s.view420.snapshot(WAGER, altered);
