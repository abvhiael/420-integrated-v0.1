...
contract BongGogglesCoreSocial420Test {
    ...
    function _friend(address a, address b) internal returns (bytes32 requestId) {
        vm.prank(a);
        requestId = graph.requestFriend(a, b);
        vm.prank(b);
    ...
    function testFriendshipRequiresRecipientAcceptanceAndIsSymmetric() public {
        vm.prank(ALICE);
        bytes32 requestId = graph.requestFriend(ALICE, BOB);
        require(!graph.areFriends(ALICE, BOB), "premature friendship");
        vm.prank(BOB);
        graph.acceptFriend(BOB, requestId);
        require(graph.areFriends(ALICE, BOB), "a-b not friends");
        require(graph.areFriends(BOB, ALICE), "b-a not friends");
    }
    ...
    function testBlockTerminatesFriendshipAndBothFollowDirections() public {
        ...
        require(!graph.isFollowing(ALICE, BOB), "outgoing follow survived");
        require(!graph.isFollowing(BOB, ALICE), "incoming follow survived");
    }
    ...
    function testMuteDoesNotDestroyRelationshipAndCanExpire() public {
        _friend(ALICE, BOB);
        vm.prank(ALICE);
        graph.muteUser(ALICE, BOB, BongGogglesTypes420.MUTE_GAME_INVITES, uint64(block.timestamp + 1 days));
    ...
    function testEditPreservesVersionHashesAndDeletePreservesObject() public {
        ...
        vm.prank(ALICE);
        bytes32 objectId = objects.publish(ALICE, BongGogglesTypes420.SocialObjectType.STATUS, bytes32(0), bytes32(0), bytes32(0), v1, bytes32(0), audience);
        vm.prank(ALICE);
        objects.edit(ALICE, objectId, v2, bytes32(0));
        ...
        require(o.exists, "delete erased provenance");
        require(o.status == BongGogglesTypes420.SocialObjectStatus.DELETED, "delete status");
    }

    function testSelfRelationshipsFailClosed() public {
        vm.expectRevert(BongGogglesRelationshipGraph420.SelfRelationship.selector);
        vm.prank(ALICE);
        graph.follow(ALICE, ALICE);
        vm.expectRevert(BongGogglesRelationshipGraph420.SelfRelationship.selector);
        vm.prank(ALICE);
        graph.requestFriend(ALICE, ALICE);
    }
}
