...
contract NativeValueCaps420 is ICapabilityRegistry420 {
    ...
    function isAuthorized(address p,bytes32 componentId,bytes32 capabilityId,bytes32,uint256 amount) external view override returns(bool){ return enabled && p==principal && componentId==ExchangeIds420.EXCHANGE_ROUTER && capabilityId==ExchangeIds420.ACTION_SWAP && amount<=100 ether; }
}
...
contract ExchangeNativeValue420Test {
    ...
    constructor() payable {
        wrapped=new Wrapped420(); token=new NativeValueToken420(); caps=new NativeValueCaps420(); referenceOracle=new NativeValueOracle420(); adapter=new NativeValueAdapter420();
        authorization=new ExchangeAuthorization420(address(caps)); emergency=new ExchangeEmergencyControl420(address(this)); oracleGuard=new ExchangeOracleGuard420(address(this)); assets=new ExchangeAssetRegistry420(address(this)); markets=new ExchangeMarketRegistry420(address(this),address(assets),T_ID); routes=new ExchangeRouteRegistry420(address(this));
        feePolicy=new ExchangeFeePolicy420(address(this)); feeRouter=new ExchangeFeeRouter420(address(this),address(feePolicy));
        _asset(W_ID,address(wrapped),bytes16("W420")); _asset(T_ID,address(token),bytes16("TEST"));
        ...
        caps.setPrincipal(address(this)); token.mint(address(adapter),100 ether); token.mint(address(this),100 ether); token.approve(address(adapter),type(uint256).max);
        if (address(this).balance >= 20 ether) { wrapped.deposit{value:10 ether}(); wrapped.transfer(address(adapter),10 ether); }
    }

    function testNativeInputWrapsExecutesAndLeavesNoResidue() public { if (address(this).balance < 1 ether) return; ExchangeAtomicRouter420.Hop[] memory hops=_toToken(); bytes32 pathHash=router.hashPath(address(wrapped),1 ether,address(this),hops); uint256 beforeToken=token.balanceOf(address(this)); uint256 out=router.swapExactInputNativePath{value:1 ether}(95e16,address(this),pathHash,hops); require(out==1 ether,"output"); require(token.balanceOf(address(this))-beforeToken==1 ether,"recipient"); require(wrapped.balanceOf(address(router))==0,"wrapped residue"); require(wrapped.allowance(address(router),address(adapter))==0,"allowance residue"); require(address(router).balance==0,"native residue"); }
    function testTokenInputUnwrapsFinalOutputAndLeavesNoResidue() public { if (wrapped.balanceOf(address(adapter)) < 1 ether) return; ExchangeAtomicRouter420.Hop[] memory hops=_toNative(); bytes32 pathHash=router.hashPath(address(token),1 ether,address(this),hops); uint256 beforeNative=address(this).balance; uint256 out=router.swapExactInputPathForNative(address(token),1 ether,95e16,address(this),pathHash,hops); require(out==1 ether,"output"); require(address(this).balance==beforeNative+1 ether,"native recipient"); require(wrapped.balanceOf(address(router))==0,"wrapped residue"); require(address(router).balance==0,"native residue"); }
    function testDirectNativeTransferIsRejected() public { if (address(this).balance == 0) return; (bool ok,)=address(router).call{value:1}(""); require(!ok,"direct native accepted"); }
    function testNativeInputOracleFailureRollsBackWrapAndValue() public { if (address(this).balance < 1 ether) return; ExchangeAtomicRouter420.Hop[] memory hops=_toToken(); bytes32 pathHash=router.hashPath(address(wrapped),1 ether,address(this),hops); uint256 beforeNative=address(this).balance; uint256 beforeSupply=wrapped.totalSupply(); referenceOracle.set(2e18); (bool ok,)=address(router).call{value:1 ether}(abi.encodeWithSelector(router.swapExactInputNativePath.selector,95e16,address(this),pathHash,hops)); require(!ok,"deviation accepted"); require(address(this).balance==beforeNative,"native not rolled back"); require(wrapped.totalSupply()==beforeSupply,"wrap not rolled back"); require(wrapped.balanceOf(address(router))==0,"router wrapped residue"); require(address(router).balance==0,"router native residue"); referenceOracle.set(1e18); }
