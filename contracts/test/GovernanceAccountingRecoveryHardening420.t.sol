...
interface VmAccountingRecovery420 { function warp(uint256) external; function getBlockTimestamp() external view returns (uint256); }
...
contract AccountingProtocolRegistryMock420 {
    ...
    function set(bytes32 id, address implementation) external {
        refs[id] = Types420.ContractRef({componentId:id, implementation:implementation, runtimeCodeHash:implementation.codehash,
            version:Types420.Version({major:1, minor:0, patch:0}), lifecycle:Types420.Lifecycle.ACTIVE});
    }
...
contract UnauthorizedAccountingCaller420 {
    function reconcile(BridgeAccountingRegistry accounting, bytes32 assetId, uint256 authorizedSupply,
        uint256 observedSupply, uint64 observedAt, bytes32 evidenceHash) external {
...
contract GovernanceAccountingRecoveryHardening420Test {
    ...
    function testUnauthorizedReconciliationRejected() public {
        ...
        (bool ok,) = address(attacker).call(abi.encodeCall(attacker.reconcile,
            (accounting, ASSET, 420, 420, observedAt, keccak256("unauthorized"))));
    ...
    function testNoncanonicalAssetRejected() public {
        ...
        (bool ok,) = address(accounting).call(abi.encodeCall(accounting.applyReconciliation,
            (unknown, 420, 420, uint64(block.timestamp), keccak256("unknown"))));
    ...
    function testStaleOrDuplicateObservationCannotOverwriteNewerEvidence() public {
        ...
        (bool duplicateOk,) = address(accounting).call(abi.encodeCall(accounting.applyReconciliation,
            (ASSET, 420, 420, nowTs, keccak256("duplicate"))));
        require(!duplicateOk, "duplicate observation overwrote evidence");
        if (nowTs > 1) {
            (bool staleOk,) = address(accounting).call(abi.encodeCall(accounting.applyReconciliation,
                (ASSET, 420, 420, nowTs - 1, keccak256("stale"))));
