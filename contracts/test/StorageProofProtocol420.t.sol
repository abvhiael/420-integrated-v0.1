...
contract StorageProofProtocol420Test {
    ...
    function setup() internal returns (Env memory e) {
        ...
        vm.prank(ALICE);
        e.providers.registerProvider(e.providerId, ALICE, keccak256("provider-meta"), keccak256("stake"));
        vm.prank(ALICE);
        e.providers.setState(e.providerId, ResourceProviderRegistry420.State.ACTIVE);
        vm.prank(ALICE);
        e.nodes.registerNode(e.nodeId, e.providerId, ResourceIds420.SERVICE_STORE, ALICE, keccak256("endpoint"), keccak256("capacity"));
        vm.prank(ALICE);
        e.nodes.setState(e.nodeId, ResourceNodeRegistry420.State.ACTIVE);
        ...
        vm.prank(GOVERNOR);
        e.schemes.registerScheme(e.schemeId, address(e.verifier), StorageProofIds420.PROOF_AVAILABILITY_WINDOW, keccak256("spec-v1"), 300);
    }
    ...
    function testCommitmentRequiresActiveStoreNodeAndProviderAuthority() public {
        ...
        vm.prank(ALICE);
        e.nodes.registerNode(relayNode, e.providerId, ResourceIds420.SERVICE_RELAY, ALICE, keccak256("relay-endpoint"), keccak256("relay-capacity"));
        vm.prank(ALICE);
        e.nodes.setState(relayNode, ResourceNodeRegistry420.State.ACTIVE);
        vm.prank(ALICE);
    ...
    function testSchemeOrNodeSuspensionStopsProofAcceptance() public {
        ...
        vm.prank(GOVERNOR);
        e.schemes.setSchemeActive(e.schemeId, false);
        ...
        vm.prank(GOVERNOR);
        e.schemes.setSchemeActive(e.schemeId, true);
        vm.prank(ALICE);
        e.nodes.setState(e.nodeId, ResourceNodeRegistry420.State.SUSPENDED);
