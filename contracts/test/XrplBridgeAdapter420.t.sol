...
contract XrplRouterCaller420 {
    function inbound(XrplBridgeAdapter420 adapter, bytes calldata proof) external returns (IBridgeAdapter420.VerifiedTransfer memory) { return adapter.verifyInbound(proof); }
    function outbound(XrplBridgeAdapter420 adapter, bytes32 routeId, bytes32 assetId, address sender, bytes calldata recipient, uint256 amount, bytes calldata extra) external returns (bytes32) { return adapter.initiateOutbound(routeId, assetId, sender, recipient, amount, extra); }
}
...
contract XrplBridgeAdapter420Test {
    ...
    function testMalformedOutboundTagFailsClosed() public {
        ...
        (bool ok,) = address(router).call(abi.encodeCall(router.outbound, (adapter, ROUTE, XRP_ASSET, address(this), recipient, 1_000_000, bytes(""))));
        require(!ok, "tag accepted");
    }

    function _setHealthy(bytes32 messageId, bytes32 transactionHash) private { verifier.set(_healthy(messageId, transactionHash)); }

    function _healthy(bytes32 messageId, bytes32 transactionHash) private pure returns (IXrplFinalityVerifier420.FinalizedTransfer memory p) {
        p = IXrplFinalityVerifier420.FinalizedTransfer({ledgerIndex: 100_000_000, ledgerHash: keccak256("validated-ledger"), transactionRoot: keccak256("transaction-root"), transactionHash: transactionHash, messageId: messageId, gatewayAccount: GATEWAY, sourceAccount: SOURCE, recipient: address(0xB0B), amountDrops: 42_000_000, destinationTag: 420, hasDestinationTag: true, validated: true, tesSuccess: true});
    }
}
