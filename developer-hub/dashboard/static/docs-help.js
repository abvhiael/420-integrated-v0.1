export function resolveDeveloperHelp(bundle, contextualId, environment, versionIntent = 'current') {
  if (!bundle || typeof bundle !== 'object') return { available: false, reason: 'invalid-bundle' };
  const id = String(contextualId || '');
  if (!id.startsWith('CTX-DEV-')) return { available: false, reason: 'invalid-contextual-id' };
  ...
  return {
    ...
    environment,
    versionIntent,
    url: base + 'versions/' + environment + '/current/' + route,
    authority: 'documentation-navigation-only'
  };
}

export async function loadDeveloperHelpBundle(fetchImpl = globalThis.fetch) {
