...
- **CMP-0.4 — Signed execution manifests:** versioned content/authorizer signature, chain/job domain, expiry, nonce, executable digest, runtime and resource bounds, partition scheme, verification and off-chain data policy. Define canonical manifest encoding/hashing and reject post-acceptance semantic mutation.
- **CMP-0.5–0.12:** provider/node/resource identity; offers/requests/matching; authorized lifecycle; Vault funding and settlement; receipts/verification; dispute/privacy/security; client/node/AI integrations; exact-head qualification and closeout. Each phase must be source grounded, separately classified as specification versus executable implementation, and preserve frozen V1 invariants.
