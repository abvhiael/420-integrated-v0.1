...
func (r RPCStorageReader) AssignmentByAgreement(ctx context.Context, agreementID string) (AssignmentSnapshot, error) {
	...
	size,sok := wordUint64(aw[10]); start,stok := wordUint64(aw[11]); end,eok := wordUint64(aw[12]); interval,iok := wordUint64(aw[13])
	if !sok||!stok||!eok||!iok||zeroWord(aw[8])||zeroWord(aw[9])||size==0||start>=end||interval==0||now.Before(time.Unix(int64(start),0).UTC())||now.After(time.Unix(int64(end),0).UTC()){ return AssignmentSnapshot{},ErrInactiveAssignment }
	...
	if !wordBool(cw[9])||!cok||!csok||!ceok||!sameWord(cw[2],aw[7])||!sameWord(cw[3],aw[3])||cs!=size||cstart!=start||cend!=end||zeroWord(cw[1]) { return AssignmentSnapshot{},ErrInvalidChainState }
	...
	if !wordBool(rw[4])||!wordBool(rw[5])||!rsok||!rok||!sameWord(rw[0],cw[1])||!sameWord(rw[1],id)||rs!=size||release<end||now.Unix()>=int64(release){return AssignmentSnapshot{},ErrInactiveAssignment}
	...
	return AssignmentSnapshot{Assignment:Assignment{AgreementID:wordHex(id),CommitmentID:wordHex(aw[8]),NodeID:wordHex(cw[1]),ShardRoot:strings.TrimPrefix(wordHex(cw[3]),"0x"),SizeBytes:size,StartTime:time.Unix(int64(start),0).UTC(),EndTime:time.Unix(int64(end),0).UTC(),Active:true},WindowCount:uint32(windows)},nil
}
...
func (r RPCStorageReader) validate()error{for _,a:=range []string{r.Contracts.Agreement,r.Contracts.Commitment,r.Contracts.Capacity,r.Contracts.Settlement,r.Contracts.Scheme}{if !validHexAddress(a){return ErrInvalidChainState}};return nil}
