...
<main id="main"><h1>{{.Heading}}</h1>{{if eq .Route "discover"}}<p>Discover public places and upcoming events across the 420 Integrated community.</p><form method="get" action="/travel"><label>Destination (city or region)<input name="destination" maxlength="80" value="{{.Destination}}"></label><button type="submit">Find places</button></form>
{{if eq .State "ready"}}<section aria-label="Public places"><h2>Public places</h2>{{range .Venues}}<article class="panel"><h3><a href="/travel/place/{{.Place.ID}}">{{.Place.Name}}</a></h3><p>{{.Place.Category}} · {{.Place.City}} {{.Place.Region}} {{.Place.Country}}</p>{{if $.SaveAvailable}}<p><a href="/travel/save/place/{{.Place.ID}}">Save {{.Place.Name}} to a trip</a></p>{{end}}{{range cannabisPlaceLabels .Place.Category}}<p>{{.}}</p>{{end}}{{if eq .Place.Kind "area"}}<p>Approximate location — exact coordinates are not published.</p>{{end}}{{if .Events}}<h4>Upcoming events</h4><ul>{{range .Events}}<li>{{.Title}} — <time datetime="{{.StartAt.Format "2006-01-02T15:04:05Z07:00"}}">{{.StartAt.Format "2 Jan 2006 15:04 MST"}}</time>{{range cannabisEventLabels .Tags}} · {{.}}{{end}}{{if and $.SaveAvailable (validEventID .ID)}} · <a href="/travel/save/event/{{.ID}}">Save event to a trip</a>{{end}}</li>{{end}}</ul>{{end}}</article>{{end}}</section>{{else}}<section class="panel" role="status" aria-live="polite"><h2>{{.Message}}</h2>{{if eq .State "disconnected"}}<p>Configure the public Location/Events service to enable discovery. No sample listings are shown as live data.</p>{{else if eq .State "empty"}}<p>No public places are available in this service response.</p>{{else if eq .State "invalid"}}<p>Enter a destination of at most 80 characters.</p>{{else}}<p>Public discovery could not be loaded. No private or cached records are shown.</p>{{end}}</section>{{end}}
...
{{if eq .State "ready"}}<section aria-label="Public events"><h2>Upcoming events</h2>{{range .Events}}<article class="panel"><h3>{{.Title}}</h3><p><time datetime="{{.StartAt.Format "2006-01-02T15:04:05Z07:00"}}">{{.StartAt.Format "2 Jan 2006 15:04 MST"}}</time></p>{{if .PlaceName}}<p>At {{if .PlaceURL}}<a href="{{.PlaceURL}}">{{.PlaceName}}</a>{{else}}{{.PlaceName}}{{end}} · {{.City}} {{.Region}}</p>{{else}}<p>No public venue is specified.</p>{{end}}{{range .Attributes}}<p>{{.}}</p>{{end}}{{if and $.SaveAvailable .ID}}<p><a href="/travel/save/event/{{.ID}}">Save {{.Title}} to a trip</a></p>{{end}}</article>{{end}}</section>{{else}}<section class="panel" role="status" aria-live="polite"><h2>{{.Message}}</h2>{{if eq .State "disconnected"}}<p>Connect the public Location/Events service to browse events.</p>{{else if eq .State "empty"}}<p>No public events matched these filters.</p>{{else if eq .State "invalid"}}<p>Choose a UTC date within the next 30 days and a destination of at most 80 characters.</p>{{else}}<p>Public events could not be loaded. No private or cached records are shown.</p>{{end}}</section>{{end}}{{if not .SaveAvailable}}<p>Saving events to trips is not connected in this public-only view.</p>{{end}}<p>Ticketing and organizer provenance are not connected in this build.</p>
...
func HandlerWithReader(reader PublicReader) http.Handler {
 ...
 mux.HandleFunc("GET /travel", func(w http.ResponseWriter, r *http.Request) {
  data := pageData{Route: "discover", Heading: "Explore somewhere new.", State: "disconnected", Message: "Discovery is not connected yet", Destination:strings.TrimSpace(r.URL.Query().Get("destination")),SaveAvailable:saveLinksAvailable(r)}
  if len(data.Destination)>80||strings.ContainsAny(data.Destination,"\x00\r\n") {data.State="invalid";data.Message="Invalid destination";renderPage(w,http.StatusBadRequest,data);return}
  if reader != nil {
   ctx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
   defer cancel()
   now := time.Now().UTC()
   feed, err := loadPublicDiscovery(ctx, reader, sdk.EventQuery{From: now, To: now.Add(30*24*time.Hour), Limit: 100})
   if err != nil { data.State = "error"; data.Message = "Discovery is temporarily unavailable" } else {
    for _,venue:=range feed.Travel {if validPlaceID(venue.Place.ID)&&(data.Destination==""||matchesDestination(venue.Place,data.Destination)) {data.Venues=append(data.Venues,venue)}}
    if len(data.Venues)==0 {data.State="empty";data.Message="No public places found"} else {data.State="ready"}
   }
  }
 ...
 mux.HandleFunc("GET /travel/map",func(w http.ResponseWriter,r *http.Request){servePublicNearby(w,r,reader)})
 ...
 mux.HandleFunc("GET /travel/place/{place_id}", func(w http.ResponseWriter, r *http.Request) {servePublicPlace(w,r,reader)})
 return mux
}
