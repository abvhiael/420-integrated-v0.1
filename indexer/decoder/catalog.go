...
// ApplyVersion applies one canonical registry publication event.
// Versions must remain monotonic per service, matching ProtocolRegistry's immutable history semantics.
func (c *Catalog) ApplyVersion(ev VersionPublished) error {
	if ev.ServiceID == "" || ev.Version == 0 || ev.Implementation == "" || ev.BlockHash == "" {
		return ErrInvalidVersionHistory
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	key := serviceKey(ev.ServiceID, ev.Version)
	if existing, ok := c.byKey[key]; ok {
		if existing.Implementation != normalizeAddress(ev.Implementation) || existing.ActivatedBlock != ev.BlockNumber || existing.ActivatedHash != ev.BlockHash {
			return ErrInvalidVersionHistory
		}
		return nil // deterministic replay/idempotence
	}
	if ev.Version > 1 {
		if _, ok := c.byKey[serviceKey(ev.ServiceID, ev.Version-1)]; !ok {
			return ErrInvalidVersionHistory
		}
	}
	impl := normalizeAddress(ev.Implementation)
	record := ServiceVersion{
		ServiceID: strings.ToLower(ev.ServiceID), Version: ev.Version, Implementation: impl,
		CodeHash: ev.CodeHash, MetadataHash: ev.MetadataHash, ActivatedBlock: ev.BlockNumber,
		ActivatedHash: ev.BlockHash, Active: ev.Active,
	}
	c.byKey[key] = record
	c.byImpl[impl] = append(c.byImpl[impl], key)
	sort.Slice(c.byImpl[impl], func(i, j int) bool {
		return c.byKey[c.byImpl[impl][i]].ActivatedBlock < c.byKey[c.byImpl[impl][j]].ActivatedBlock
	})
	return nil
}
...
func (c *Catalog) ApplyProfile(ev ProfilePublished) error {
	...
	record.ComponentType = ev.ComponentType
	record.ManifestHash = ev.ManifestHash
	record.DependencyRoot = ev.DependencyRoot
	record.InterfaceHash = ev.InterfaceHash
	c.byKey[key] = record
	return nil
}
...
func (c *Catalog) ApplyDeprecated(serviceID string, version uint32, blockNumber uint64) error {
	...
	record.Active = false
	record.DeprecatedBlock = blockNumber
	c.byKey[key] = record
	return nil
}
...
// ResolveImplementationAt resolves the service/version that owned an implementation at a specific block.
// This is what keeps historical decoding pinned to the version that actually emitted the log.
func (c *Catalog) ResolveImplementationAt(implementation string, blockNumber uint64) (ServiceVersion, error) {
	...
	for _, key := range keys {
		...
		if !found || record.ActivatedBlock > best.ActivatedBlock {
