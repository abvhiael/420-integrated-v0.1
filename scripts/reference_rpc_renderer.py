...
def parse_forbidden(text: str) -> tuple[list[str], list[str]]:
    ...
    return re.findall(r"'([^']+)'", prefix_match.group(1)), re.findall(r"'([^']+)'", explicit_match.group(1))
...
def parse_policy(text: str) -> tuple[bool, bool, list[str], list[str]]:
    ...
    subs = re.search(r"kind !== 'newHeads' && kind !== 'logs' && kind !== 'newPendingTransactions' && kind !== 'syncing'", text)
    if not allow or not positional or not tags or not subs:
        raise ValueError("420RPC request-policy constants not found")
    return allow.group(1) == "true", positional.group(1) == "true", re.findall(r"'([^']+)'", tags.group(1)), ["newHeads", "logs", "newPendingTransactions", "syncing"]


def parameter_rule(method: str) -> str:
    rules = {
        "web3_clientVersion": "[]", "net_version": "[]", "eth_chainId": "[]", "eth_syncing": "[]", "eth_blockNumber": "[]", "eth_gasPrice": "[]", "eth_maxPriorityFeePerGas": "[]",
        "eth_getBalance": "[address, block]", "eth_getCode": "[address, block]", "eth_getTransactionCount": "[address, block]", "eth_getStorageAt": "[address, position, block]",
        "eth_getBlockByHash": "[hash, fullTransactions]", "eth_getBlockByNumber": "[block, fullTransactions]", "eth_getBlockTransactionCountByHash": "[hash]", "eth_getTransactionByHash": "[hash]", "eth_getTransactionReceipt": "[hash]", "eth_getBlockTransactionCountByNumber": "[block]",
        "eth_getTransactionByBlockHashAndIndex": "[hash, index]", "eth_getTransactionByBlockNumberAndIndex": "[block, index]", "eth_getLogs": "[filter]", "eth_call": "[transaction, block] (+ optional state override)", "eth_estimateGas": "[transaction] (+ optional block)", "eth_feeHistory": "[blockCount, newestBlock, rewardPercentiles]", "eth_sendRawTransaction": "[signedTransactionBytes]", "eth_subscribe": "[subscriptionType] (+ optional logs filter)", "eth_unsubscribe": "[subscriptionId]",
    }
...
def render() -> str:
    ...
    lines = [
        "---", "title: Generated public RPC reference", "audience:", "  - developer", "category: reference", "status: generated", "version: current", "---", "",
        "# Generated public RPC reference", "", "> GENERATED FILE - DO NOT EDIT. Regenerate from `420-rpc/src/methods.ts` and `420-rpc/src/request-policy.ts`.", "",
        "Method source: `420-rpc/src/methods.ts`  ", "Request-policy source: `420-rpc/src/request-policy.ts`", "",
        ...
        "## Public compatibility surface", "", "| Method | Profile | Transport | Parameters | Required upstream capability | Mutates chain | Requires user signature |", "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for item in methods:
        lines.append(f"| `{item['method']}` | `{item['profile']}` | `{item['transport']}` | `{parameter_rule(item['method'])}` | `{item['capabilities']}` | {item['mutates']} | {item['signature']} |")
    lines += [
        ...
        "- Invalid request envelope: `-32600`", "- Method outside the public compatibility surface: `-32601`", "- Invalid method parameters: `-32602`",
        ...
        "", "## Deliberately excluded public surface", "", "420RPC fails closed instead of forwarding arbitrary upstream methods.", "",
        ...
        "- Private Engine API is not part of this public reference.", "- Node-managed accounts, node-side signing, admin/debug/miner/txpool access are not public 420RPC capabilities.",
