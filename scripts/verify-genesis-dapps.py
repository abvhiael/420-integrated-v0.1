...
required_files = [
    ...
    "contracts/config/420explorer-genesis.json", "docs/420EXPLORER.md",
    "contracts/config/420search-genesis.json", "docs/420SEARCH.md", "testnet/public-services/search/readiness.json",
    "contracts/config/420analytics-genesis.json", "docs/420ANALYTICS.md", "testnet/public-services/analytics/readiness.json",
    ...
    "contracts/src/token/TokenIds420.sol", "contracts/src/token/TokenTemplateRegistry420.sol", "contracts/src/token/TokenFactory420.sol", "contracts/src/token/ERC20Template420.sol", "contracts/src/token/ERC721Template420.sol", "contracts/src/token/ERC1155Template420.sol",
]
for path in required_files:
    if not (root / path).exists():
        errors.append("missing " + path)

for name in ["420 Wallet", "420 Explorer", "420 Search", "420 Analytics", "420 AppStore", "420 Verify", "420 Notifications", "420 Status"]:
...
notifications = load("contracts/config/420notifications-genesis.json")
if notifications:
    subscriptions = notifications.get("subscriptionModel", {})
    delivery = notifications.get("delivery", {})
    authority = notifications.get("authority", {})
    privacy = notifications.get("privacy", {})
    integrity = notifications.get("integrity", {})
    if notifications.get("contractsRequired") is not False or notifications.get("canonicalStateAuthority") is not False or notifications.get("serviceId") != "420/service/notifications/v1":
        errors.append("notifications authority/service invariant missing")
    require_true(subscriptions, ["optInByDefault", "userCanMutePerSource", "userCanMutePerTopic", "userCanSetSeverityThreshold", "userCanSetDeliveryChannels", "userCanUnsubscribe", "subscriptionStateMayRemainClientLocal", "promotionalNotificationsRequireSeparateConsent"], "notifications subscription invariant missing")
    require_true(delivery, ["deduplicationRequired", "rateLimitsRequired", "priorityAndSeveritySupported", "retryMustNotDuplicateUserVisibleEvent", "deliveryFailureDoesNotChangeCanonicalState", "deepLinksMustPreserveCanonicalSource", "timestampsAndNetworkIdentityRequired"], "notifications delivery invariant missing")
    require_false(authority, ["canSignTransactions", "canGrantCapabilities", "canApproveSpending", "canMutateProtocolState", "canCreateCanonicalEvents", "canOverrideWalletConfirmation"], "notifications authority invariant missing")
    require_true(authority, ["actionButtonsRequireWalletOrOriginAppAuthorization"], "notifications action authorization invariant missing")
...
out = {
    ...
    "notifications_profile": "420-notifications-genesis-v1",
