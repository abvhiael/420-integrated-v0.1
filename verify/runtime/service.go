...
type ChainProbe interface {
	...
	CanonicalBytecodeAvailable(context.Context) error
}
...
func (s *Service) Qualify(ctx context.Context) error {
	...
	if err := s.probe.CanonicalBytecodeAvailable(ctx); err != nil {
		s.ready.Store(false)
		return fmt.Errorf("canonical bytecode unavailable: %w", err)
	}
	s.ready.Store(true)
	return nil
}
