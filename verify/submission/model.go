...
func NewStandardJSON(raw []byte, build BuildSettings) (Submission, error) {
	...
	s.BundleHash = hashParts([]part{{name: "standard-json", content: canonical}})
	return s, nil
}
...
func NewMultiFile(files map[string]string, build BuildSettings) (Submission, error) {
	...
	if len(files) == 0 {
		return Submission{}, errors.New("at least one source file is required")
	}
...
func NewFlattened(filename, source string, build BuildSettings) (Submission, error) {
	...
	if source == "" {
		return Submission{}, errors.New("flattened source is required")
	}
...
func (b BuildSettings) Validate() error {
	if strings.TrimSpace(b.CompilerVersion) == "" {
		return errors.New("compiler version is required")
	}
	if strings.TrimSpace(b.EVMVersion) == "" {
		return errors.New("evm version is required")
	}
	if strings.TrimSpace(b.MetadataHashMode) == "" {
		return errors.New("metadata hash mode is required")
	}
	for _, l := range b.Libraries {
		if err := validatePath(l.Source); err != nil {
			return fmt.Errorf("library source: %w", err)
		}
		if strings.TrimSpace(l.Library) == "" {
			return errors.New("library name is required")
		}
		if !validAddress(l.Address) {
			return fmt.Errorf("invalid library address for %s", l.Library)
		}
	}
	if b.ConstructorArgsKnown && !validHexBytes(b.ConstructorArguments) {
		return errors.New("known constructor arguments must be 0x-prefixed hex bytes")
	}
	return nil
}
...
func hashParts(parts []part) string {
	...
	for _, p := range parts {
		...
		h.Write(n[:])
		h.Write([]byte(p.name))
		binary.BigEndian.PutUint64(n[:], uint64(len(p.content)))
		h.Write(n[:])
		h.Write(p.content)
	}
...
func validatePath(p string) error {
	...
	if p == "" || strings.HasPrefix(p, "/") || strings.Contains(p, "\\") || strings.Contains(p, "../") || p == ".." {
		return errors.New("source path must be relative and traversal-free")
	}
	return nil
}
