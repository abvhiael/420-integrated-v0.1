...
const record = (recipient, owner, expiry) => `0x${addressWord(owner)}${word(0)}${addressWord(recipient)}${word(0)}${word(0)}${word(expiry)}${word(5)}`;
function setup() {
  let chain = '0x1';
  ...
  const provider = { async request(method, params = []) {
    ...
    if (method === 'eth_call') {
      ...
      if (params[0].data.startsWith(selector('resolve(bytes32)'))) return record(target, owner, expiry);
    }
  ...
  return { provider, config, retarget(value) { target = value; }, reassign(value) { owner = value; }, renew(value) { expiry = value; }, switchChain(value) { chain = value; } };
}
...
test('name owner or lease renewal changes invalidate even unchanged recipient addresses', async () => {
  const rpc = setup();
  const flow = createQualifiedNamesSend420({ provider: rpc.provider, config: rpc.config, confirm: () => true });
  const prepared = await flow.prepare({ recipient: 'alice.420', amount: '1', asset: { kind: 'native' } });
  rpc.reassign(OTHER);
